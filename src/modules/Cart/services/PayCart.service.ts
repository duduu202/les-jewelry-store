...
import { Cart_status, Paid_status, Coupon, Coupon_type, Address } from '@prisma/client';
...
class PayCartService {
  ...
  public async execute({ ...cartParams }: IPayCartDTO): Promise<Cart> {
    ...
    const validated_cards = await this.checkPaymentCardSplit(payment_cards, total_value);

    const updated_cart = await this.confirmPayment({cart, validated_cards, products, total_value, discount, coupon, address});
  ...
  private async checkCupom(coupon_code: string, user_id: string): Promise<Coupon> {
    ...
    if(new Date(coupon.expires_at) < new Date()) throw new AppError('Cupom expirado', 400);
    if(coupon.quantity <= 0) throw new AppError('Cupom esgotado', 400);
    return coupon;
  }

  private async checkPaymentCardSplit(payment_cards: {card_id: string, percentage: number}[], card_value: number): Promise<{payment_card: PaymentCard, percentage: number}[]> {
    if(payment_cards.length == 0) throw new AppError('Nenhum cartão selecionado', 400);
    ...
    const total_percentage = validated_cards.reduce((total, card) => total + card.percentage, 0);
    if(total_percentage != 100) throw new AppError('A soma das porcentagens do cartão deve ser 100%', 400);

    // min 10 reais
    const min_value = 10;
    const min_value_percentage = min_value / card_value;  
    validated_cards.forEach((card) => {
      if(card.percentage < min_value_percentage) throw new AppError('Cada cartão deve pagar no minimo 10 reais', 400);
    });

    return validated_cards;
  }

  private async checkProducts(cart: Cart): Promise<{product: Product, quantity: number}[]> {
    const products = await Promise.all(
      cart.cart_items.map(async (item) => {
        ...
        if(product.stock < item.quantity) throw new AppError('Quantidade de produto indisponível', 400);
